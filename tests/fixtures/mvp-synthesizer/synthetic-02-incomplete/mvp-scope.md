...
## Problem Restatement
FreshMart store managers lack real-time operational visibility across inventory, sales, and
scheduling. The dashboards, inventory alerts, and scheduling tools are not independent features —
they are symptoms of a single gap: operational blindness at the store level. RetailEdge addresses
this as one integrated platform decision.
...
**Rationale:** No confirmed delivery deadline exists; stakeholder buy-in is the critical constraint.
Scoping to highest business value for store managers anchors the MVP to demonstrable impact.
...
| Inventory Alerts | Low-stock and expiry alerts by SKU | HIGH | Named in brief, high confidence, central to store-level visibility |

## Scope: Out
| Feature | Why Out | Deferred to |
|---|---|---|
| Staff Scheduling | MED confidence; valuable but lower immediate impact than visibility tools | Phase 2 |
| Mobile App | No platform decision (iOS/Android/both); native build cost outweighs MVP value | Phase 2 |
| Supplier Reorder | LOW confidence; single mention, no stakeholder confirmation; depends on Inventory Alerts first | Phase 2 |
...
## Source Artifacts
- discovery.md — RetailEdge: 5 features extracted, 2 users identified, 1 unresolved tech CONFLICT,
  no timeline; core problem confirmed in MVP_SYNTHESIZER session
